...
$FunctionDir = Join-Path $RepoRoot "functions\weather\taf\processor"
$SharedWeatherDir = Join-Path $RepoRoot "functions\shared\wilvor_weather"

if (-not (Test-Path $FunctionDir)) {
    throw "TAF processor directory not found: $FunctionDir"
}
...
try {
    ...
    Compress-Archive `
        -Path package\* `
        -DestinationPath dist\taf_processor.zip `
        -Force

    Write-Host "Built: $FunctionDir\dist\taf_processor.zip"
}
finally {
    Pop-Location
}
