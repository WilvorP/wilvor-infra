...
def fetch_noaa_metars() -> Any:
    url = os.environ["NOAA_METAR_URL"]

    request = urllib.request.Request(
        url,
        method="GET",
        headers={
            "User-Agent": "Wilvor-METAR-Poller/0.1",
    ...
    try:
        ...
        raise RuntimeError(
            f"NOAA METAR API returned HTTP {exc.code}: {exc.reason}"
        ) from exc
    except urllib.error.URLError as exc:
        raise RuntimeError(f"NOAA METAR API request failed: {exc.reason}") from exc

    if status != 200:
        raise RuntimeError(f"NOAA METAR API returned unexpected status {status}")
...
def archive_raw_response(
    *,
    ...
    raw_prefix = os.environ.get("RAW_PREFIX", "raw/source=metar")
...
def publish_raw_records(
    *,
    ...
    stream_name = os.environ["METAR_RAW_STREAM_NAME"]
