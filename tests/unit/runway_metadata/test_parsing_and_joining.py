...
def test_parse_and_join_supported_airports(
    fixtures_dir: Path,
) -> None:
...
def test_source_record_hash_is_stable(
    fixtures_dir: Path,
) -> None:
    first = parse_faa_directory(
        fixtures_dir,
        supported_airport_ids={"KSFO"},
    )

    second = parse_faa_directory(
        fixtures_dir,
        supported_airport_ids={"KSFO"},
    )
    ...
    assert [
        ...
        for item in first.runways
    ] == [
        item.source_record_hash
        for item in second.runways
    ]


def test_unsupported_airports_are_skipped_not_rejected(
    fixtures_dir: Path,
) -> None:
    result = parse_faa_directory(
        fixtures_dir,
