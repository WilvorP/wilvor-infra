...
def test_active_recommendations_list_returns_current_set_only(
    ...
    listing = repo.list_active_recommendations(limit=10)

    assert listing["count"] == 1
    assert listing["items"][0]["recommendation_id"] == "rec-current"
    assert listing["nextToken"] is None


def test_active_recommendations_list_paginates_current_set(
    operational_repository,
    monkeypatch,
):
    repo = operational_repository
    ...
    first = repo.list_active_recommendations(limit=1)

    assert first["count"] == 1
    assert first["items"][0]["recommendation_id"] == "rec-newer"
    assert first["nextToken"] is not None

    second = repo.list_active_recommendations(
        limit=1,
        next_token=first["nextToken"],
    )

    assert second["count"] == 1
    assert second["items"][0]["recommendation_id"] == "rec-older"
